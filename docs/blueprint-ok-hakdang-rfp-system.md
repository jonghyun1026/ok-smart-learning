...
- 동점 처리: RFP에 명시된 규칙이 없어 최초에는 OK뱅크 예시를 참고해 "운영부문 → 콘텐츠부문" 순으로 재비교하기로 가정했으나, 2026-07-07 사용자 확정으로 "가격평가 점수가 높은 업체 우선"으로 변경 (구현 시 담당자 최종 확인 필요 — 가정임을 CLAUDE.md에 명시)
...
| ③ 콘텐츠 | 4 | 보유 콘텐츠 양·다양성(직무/리더십/어학 등) | 5 | 제안서 콘텐츠 목록 |
| ③ 콘텐츠 | 5 | 콘텐츠 최신성 및 업데이트 주기(법정필수교육 포함) | 5 | 제안서 |
| ③ 콘텐츠 | 6 | 생성형 AI·데이터 리터러시 등 신기술 콘텐츠 보유/개발계획 | 10 | 제안서 |
...
| 2026-07-07 | 콘텐츠부문 내부 배점 재조정(보유콘텐츠 8→5, 최신성 6→5, 생성형AI 6→10, 무상탑재 5 유지, 부문 합계 25 동일) + 동점처리 규칙을 "운영→콘텐츠부문"에서 "가격평가 점수 높은 업체 우선"으로 변경 | 사용자가 "AI 관련 콘텐츠와 법정필수 결과관리 편의성이 중요"하다며 항목별 재배분을 요청(부문 총점은 유지, 항목 간 이동만), 별도로 공문체 수정본 검토 과정에서 동점처리 기준을 가격평가로 명시 변경하기로 확정 |
