...
OK금융그룹 「2026~2027년 OK학당 스마트러닝 교육 위탁운영 용역」계약을 위해, 입찰공고 게시 → RFP 열람 → 참가업체 등록(오프라인 접수) → 평가자 점수 입력 → 자동 집계·협상적격자 산출을 수행하는 웹 시스템. 설계 근거는 `docs/blueprint-ok-hakdang-rfp-system.md` (구현 전 반드시 먼저 확인).
...
1. **구현 전에 생각하라** — 배점 배분·가격산식·동점규칙(가격평가 점수 높은 업체 우선, 2026-07-07부터)은 이번 설계에서 가정한 값이다. 실사용 전 담당자 검수 필요함을 숨기지 마라.
