  ...
  return [
    new Paragraph({
      ...
      children: [
        ...
        new TextRun({ text: points ? `  (${points}점)` : "  (Pass/Fail)", bold: true, size: 20, color: "595959" }),
      ],
    }),
    gradeTable(rows),
  ];
  ...
  sections: [
    {
      ...
      children: [
        ...
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 160 }, children: [
          new TextRun({ text: "세부 평가기준 안내", bold: true, size: 52, color: DARK }),
        ] }),
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 500 }, children: [
          new TextRun({ text: "2025~2026년 OK학당 스마트러닝 교육 위탁운영 용역", size: 24, color: NAVY, bold: true }),
        ] }),

        infoTable([
          ["사업명", "2025~2026년 OK학당 스마트러닝 교육 위탁운영 용역"],
          ["계약기간", "2025.9.1 ~ 2026.8.31"],
        ...
        h1("1. 안내 개요 및 목적"),
        p(
          "OK금융그룹은 「2025~2026년 OK학당 스마트러닝 교육 위탁운영 용역」계약을 위해 공개경쟁입찰을 실시합니다. 본 문서는 참여 업체가 제안서 작성에 필요한 제출서류와 세부 평가기준을 명확히 이해할 수 있도록 안내하기 위해 작성되었습니다."
        ),
        h2("1-1. 제출서류 목록 (필수)"),
        docListTable([
        ...
        ...criterionBlock("3-1", "경영현황 및 재무안정성", 4, {
          ...
          mid: "재무지표가 업계 평균 수준이며 특이 위험신호는 확인되지 않음",
        ...
        ...criterionBlock("3-3", "전담인력 배치계획 (PM/R&R)", 2, {
          ...
          mid: "인력 구성은 제시되었으나 역할 분담이 다소 불명확함",
          low: "투입 인력 정보가 부족하거나 겸직/파트타임 위주로 구성되어 책임소재가 불분명함",
        }, "참여인력 구성도"),

        h1("4. 콘텐츠부문 (25점)"),
        ...criterionBlock("4-1", "보유 콘텐츠 양·다양성 (직무/리더십/어학 등)", 8, {
        ...
        ...criterionBlock("4-2", "콘텐츠 최신성 및 업데이트 주기 (법정필수교육 포함)", 6, {
        ...
        ...criterionBlock("4-3", "생성형 AI·데이터 리터러시 등 신기술 콘텐츠 보유/개발계획", 6, {
          high: "생성형 AI·데이터 리터러시 관련 콘텐츠를 이미 운영 중이며 향후 개발 로드맵이 구체적으로 제시됨",
          mid: "관련 콘텐츠 도입 계획은 있으나 아직 운영 실적이 없음",
        ...
        ...criterionBlock("6-2", "해외법인 (인도네시아·캄보디아 등) 접속 지원 등 안정성", 5, {
          high: "해당 국가명을 특정해 접속 안정성 대책(CDN 등)과 가용률·응답속도 수치가 제시됨",
        ...
        h1("7. 가격평가 (20점)"),
        h2("7-1. 입찰가격 환산 점수"),
        p("입찰가격은 아래 산식에 따라 자동 환산되며, 별도의 정성평가 없이 산식에 따라 객관적으로 산정됩니다.", { bold: true }),
        infoTable(
          [
        ...
        infoTable(
          [
            ...
            ["동점 처리", "총점이 동일할 경우 운영부문 점수가 높은 업체를 우선하며, 그래도 동점이면 콘텐츠부문 점수로 재비교"],
          ],
          [2000, 7360]
        ),
        ...
        infoTable(
          [
            ["입찰공고 게시", "2026.07.07"],
            ["입찰공고 마감", "~2026.07.16"],
            ["제안서·견적서 비교, 업체평가 및 선정", "~2026.07.31"],
            ["계약서 날인", "~2026.08.29"],
          ],
          [3200, 6160]
        ),
        ...
        ...[
          ...
          "세부 평가기준(배점 포함)은 이번 안내를 위해 발주기관이 신규 도출한 기준이며, 발주기관 사정에 따라 사전 공지 후 변경될 수 있습니다.",
        ].map(
          (text) =>
            new Paragraph({
